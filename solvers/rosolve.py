...
def rs_get_token(blob, proxy, api_key):
    challengeInfo = {
    ...
    try:
        response = requests.post(f'{API_BASE}/createTask', json=payload, timeout=120, proxies=proxies)
        if response.status_code == 523:
            return None, "Rosolve server is unreachable (HTTP 523 - Cloudflare error). Service may be down."
    ...
    for attempt in range(max_attempts):
        try:
            solution_response = requests.get(f'{API_BASE}/taskResult/{task_id}', timeout=30)
            ...
            return None, f"Failed to solve captcha! Code: 02 Reason: {e}"
        
        status = solution.get('status')
        
        if status == 'failed':
            return None, "Failed to solve captcha! Server returned failed status."
